...
exports.Home = catchAsyncErrors(async (req, res, next) => {
  ...
  res.json({message: 'Welcome to Vaidik Anushthanam'});
});
...
exports.userSignup = catchAsyncErrors(async (req, res, next) => {
  ...
  if (userExists) {
    return next(
      new ErrorHandler("User already exists with this email address")
    );
  }
...
exports.userSendMail = catchAsyncErrors(async (req, res, next) => {
  const subject = "New Puja Booking";
  const html = `<h1>Puja Name : ${req.body.puja}</h1> <br />
        <h1>Name : ${req.body.name}</h1> <br />
        <h1>Email : ${req.body.email}</h1> <br />
        <h1>Phone No : ${req.body.phone}</h1> <br />
        <h1>MSG : ${req.body.msg}</h1> <br />
      </div>`;
  sendMail(req, res, next, html, subject);

  res.json({ message: "Puja Book Successfully" });
});


exports.userSendMailContact = catchAsyncErrors(async (req, res, next) => {
  const subject = "User Trying To Contact";
  const html = `<h1>Name : ${req.body.name} ${req.body.lastname}</h1> <br />
        <h1>Email : ${req.body.email}</h1> <br />
        <h1>Phone No : ${req.body.phone}</h1> <br />
        <h1>MSG : ${req.body.msg}</h1> <br />
      </div>`;
